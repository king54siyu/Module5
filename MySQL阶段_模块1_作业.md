...
-- 查询练习
	...
	#12.查询含有'霸'字的所有商品
    select * from product where pname like '%霸%'; (using like is very important!!)
	#13.查询以'香'开头的所有商品
...
-- 分组练习：
    ...
    select count(category_name) from product where category_name is not null group by category_name;
```
...
select e.dept_id, d.name from employee e, dept d where e.dept_id = d.id and e.salary > 5000;
4) 查询开发部与财务部所有的员工信息，分别使用子查询和表连接实现
select * from employee e left join dept d on e.dept_id = d.id where d.name in ('财务部', '开发部');
select * from employee e where e.dept_id in (select id from dept where name in ('财务部', '开发部'));
5) 查询2011年以后入职的员工信息和部门信息，分别使用子查询和表连接实现
select * from employee e left join dept d on e.dept_id = d.id where e.join_date > '2011-01-01';
select * from employee e left join dept d on e.dept_id = d.id where join_date in (select join_date from employee where join_date > '2011-01-01')
```
...
-- 1.2 增加条件：平均成绩大于70
select s.id, s.NAME, avg(score) from student s left join student_course sc on s.id = sc.student_id group by id having avg(score) > 70;
...
select s.id, s.NAME, count(course_id), sum(score) from student s left join student_course sc on s.id = sc.student_id group by id;
...
select s.id, s.NAME from student s left join student_course sc on s.id = sc.student_id
where sc.course_id = (select c.id from course c left join teacher t on c.teacher_id = t.id where t.id = (select id from teacher where NAME = '赵云'));
...
where id in (select s.id from student s left join student_course sc on s.id = sc.student_id group by s.id having count(course_id) <= 3);
```
...
create index lname_index on lecturer(lname);

create table class_classify (
    cid int primary key auto_increment,
    cname varchar(20),
    cdesp varchar(50),
    creaete_time datetime,
...
create table class (
    ...
    history_time bigint,
    status char(1),
    foreign key (lecturer_id) references lecturer(lid),
    foreign key (class_id) references class_classify(cid)
);
create index title_index on class(title);
...
```mysql
select * from class_classify
where cid = (select class_id from lecturer l left join class c on l.lid = c.lecturer_id where l.lname = '刘德华');
```
